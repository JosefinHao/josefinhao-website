...
(function() {
    ...
    function hitWand(e) {
        ...
        // Transform origin is at 50% and ~30.6% of height (137px/448px desktop, 85px/280px mobile)
        const originX = rect.width / 2;
        const originY = rect.height * 0.306; // Stick end position after all SVG transforms (internal -25deg + .wand-image 115deg)
    ...
    function startYarnGame() {
        ...
        // Spawn 3 balls immediately for ultra fast start
        spawnYarnBall();
        setTimeout(() => spawnYarnBall(), 250);
        setTimeout(() => spawnYarnBall(), 500);

        // Spawn balls periodically - ultra fast spawn rate (214% faster than original)
        yarnGame.spawnInterval = setInterval(() => {
            if (yarnGame.isPlaying) {
                spawnYarnBall();
            }
        }, 491); // Ultra fast spawn rate (0.491 seconds, 214% faster than 1.54s)
    ...
    function spawnYarnBall() {
        ...
        // Ultra fast speed (186% faster than original) with balanced difficulty increase
        const baseSpeed = 1.713 + Math.random() * 0.857; // Ultra fast speed: 1.713-2.57 (186% faster than 0.6-0.9)
            ...
            if (dist < clickRadius) {
                ...
                if (yarnGame.score % 3 === 0) {
                    ...
                    if (yarnGame.spawnInterval) {
                        ...
                        const newInterval = Math.max(267, 491 - yarnGame.difficulty * 88);
                        yarnGame.spawnInterval = setInterval(() => {
                            if (yarnGame.isPlaying) {
                                spawnYarnBall();
                            }
    ...
    function yarnGameLoop() {
        ...
        for (let i = yarnGame.balls.length - 1; i >= 0; i--) {
            ...
            if (ball.flyingAway) {
                ...
                ball.flyVelocityY += 1.099; // Gravity pulls it down eventually (120% faster)
                ball.rotation += ball.rotationSpeed * 3; // Spin faster when flying
                ball.opacity -= 0.044; // Fade out (120% faster)
